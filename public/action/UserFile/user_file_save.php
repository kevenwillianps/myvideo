...
try{
    ...
    $user_file_id    = isset($inputs['inputs']['user_file_id'])    ? (int)$Main->anti_injection($inputs['inputs']['user_file_id'])    : 0;
    ...
    if($user_file_id < 0){
        ...
        $error++;
        $message[0] = $error. 'user_file_id - O seguinte campo deve ser preenchido/selecionado';
        ...
        if (is_dir($path)){
            ...
            if ($pointer == ($length - 1)){
                ...
                if (is_file($path.'/'.$name)){
                    ...
                    $UserFile->Save($user_file_id, $user_id, $situation_id, $name, $description, $base64, $file_type, $path, $date_register, $date_update);

                    /** Result **/
                    $result = array("cod" => 99, "message" => "Arquivo salvo com sucesso!", "user_file_id" => $UserFile->GetLast()->user_file_id);
