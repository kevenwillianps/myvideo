...
try{
   ...
   **/
   ...
   $div .= '     <a href="#" class="list-group-item list-group-item-action" style="border-left: solid 4px #51c28d;" onclick="request(\'TABLE=USER_FILE&ACTION=DATAGRID\')">';
   $div .= '         <div class="d-flex w-100 justify-content-between">';
   $div .= '             <h6 class="mb-1">listagem de UserFile</h6>';
   $div .= '         </div>';
   ...
   $div .= '   <div class="card-body">';
   $div .= '       <form role="form" id="frmUserFile">';
   $div .= '           <div class="row">';

   /** Monto meu campo 'user_file_id' **/
   $div .= '               <div class="col-md-12">';
   $div .= '                   <div class="form-group">';
   $div .= '	                    <label for="user_file_id">';
   $div .= '			                user_file_id';
   $div .= '			            </label>';
   $div .= '			            <input id="user_file_id" name="user_file_id" class="form-control" type="text" value="'.@utf8_encode($row->user_file_id).'" >';
   $div .= '	                </div>';
   $div .= '	            </div>';
   ...
   $div .= '               <div class="col-md-12">';
   $div .= '                   <div class="form-group">';
   $div .= '				        <button class="btn btn-outline-primary" type="button" onclick="sendForm(\'#frmUserFile\', \'N\')">Salvar <i class="fas fa-paper-plane"></i></button>'; 
   $div .= '	                </div>';
   $div .= '	            </div>';
   $div .= '           </div>';

   /** Monto meus campos hidden **/
   $div .= '		    <input type="hidden" name="TABLE" value="USER_FILE"/>';
   $div .= '		    <input type="hidden" name="ACTION" value="SAVE"/>';
   $div .= '			<input type="hidden" name="USER_FILE_ID" value="'.@$row->user_file_id.'"/>';
