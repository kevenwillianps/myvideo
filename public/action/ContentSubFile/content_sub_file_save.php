...
try{
   ...
   **/
   ...
   $content_sub_file_id = isset($inputs['inputs']['content_sub_file_id']) ? (int)$Main->anti_injection($inputs['inputs']['content_sub_file_id']) : 0;
        ...
        if (is_dir($path)){
            ...
            if ($pointer == ($length - 1)){
                ...
                if (is_file($path.'/'.$name)){
                    ...
                    $ContentSubFile->Save($content_sub_file_id, $content_sub_id, $user_id, $situation_id, $name, $description, $base64, $file_type, $path, $date_register, $date_update);

                    /** Result **/
                    $result = array("cod" => 99, "message" => "Arquivo salvo com sucesso!", "content_sub_file_id" => $ContentSubFile->GetLast()->content_sub_file_id);
            ...
            if (is_dir($path)){
                ...
                if ($pointer == ($length - 1)){
                    ...
                    if (is_file($path.'/'.$name)){
                        ...
                        /** Result **/
                        $result = array("cod" => 99, "message" => "Arquivo salvo com sucesso!", "content_sub_file_id" => $ContentFile->GetLast()->content_sub_file_id);
