...
try{
   ...
   **/
   ...
   /* Verifico se o campo content_file_id foi preenchido */
   if(empty($content_file_id)){

       $error++;
       $message .= $error. ' - O seguinte campo deve ser preenchido/selecionado <strong>content_file_id</strong>.<br/>';
   ...
   if($error > 0){
       ...
       if(isset($row)){
           ...
           if($row->content_file_id > 0){

               /** Executo o método de exclusão **/
               if($ContentFile->Delete($row->content_file_id)){

                   /** Se o registro possuir arquivo, sua pasta com os arquivos **/
                   if (is_dir("images/ContentFile/" . $content_file_id . "/")){
                       ...
                       delTree("images/ContentFile/" . $content_file_id . "/");

                       /** Result **/
                       $result = array("cod"   => 1,
                                       "msg"   => "Registro Excluído com Sucesso!",
                                       "title" => "Atenção",
                                       "qs"    => "TABLE=CONTENT_FILE&ACTION=DATAGRID");
