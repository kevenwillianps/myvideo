...
try{
   ...
   **/
   ...
   $content_sub_auxiliary_id = isset($inputs['inputs']['content_sub_auxiliary_id']) ? (string)$Main->anti_injection($inputs['inputs']['content_sub_auxiliary_id']) : '';
   ...
   if($content_id < 0){
       ...
       $error++;
       $message[0] = $error. '- ContentID - O seguinte campo deve ser preenchido/selecionado';
   ...
   if($error > 0){
       ...
       $ContentSub->Save($content_sub_id, $content_id, $content_sub_auxiliary_id, $user_id, $situation_id, $menu_position, $highlighter_id, $url, $title, $title_menu, $description, $content_resume, $content_complete, $date_register, $date_update);
