   ...
   **/
   ...
   class ContentFile
   {
       ...
       public function Count()
       {
           ...
           /** Consulta SQL **/
           $sql = "select count(*) as qtde from content_file ";
       ...
       public function Get($content_file_id)
       {
           ...
           /** Consulta SQL **/
           $sql = "select * from content_file where content_file_id = {$this->content_file_id}";
       ...
       public function GetLast()
       {
           ...
           /** Consulta SQL **/
           $sql = "select * from content_file order by content_file_id desc limit 1";
       ...
       public function Save($content_file_id, $content_id, $user_id, $situation_id, $name, $description, $base64, $file_type, $path, $date_register, $date_update)
       {
           ...
           if($this->content_file_id == 0)
           {
               ...
               /** Consulta SQL **/
               $sql = "INSERT INTO content_file(content_file_id, content_id, user_id, situation_id, name, description, base64, file_type, path, date_register, date_update)VALUES('{$this->content_file_id}', '{$this->content_id}', '{$this->user_id}', '{$this->situation_id}', '{$this->name}', '{$this->description}', '{$this->base64}', '{$this->file_type}', '{$this->path}', '{$this->date_register}', '{$this->date_update}')";

           }else
           {

               /** Consulta SQL **/
               $sql = "UPDATE content_file set content_file_id = '{$this->content_file_id}', content_id = '{$this->content_id}', user_id = '{$this->user_id}', situation_id = '{$this->situation_id}', name = '{$this->name}', description = '{$this->description}', base64 = '{$this->base64}', file_type = '{$this->file_type}', path = '{$this->path}', date_register = '{$this->date_register}', date_update = '{$this->date_update}' WHERE content_file_id = {$this->content_file_id}";
       ...
       public function Delete($content_file_id)
       {
           ...
           /** Consulta SQL **/
           $sql = "DELETE FROM content_file WHERE content_file_id = {$this->content_file_id}";
