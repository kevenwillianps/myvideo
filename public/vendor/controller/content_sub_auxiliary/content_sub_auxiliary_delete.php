...
try {
    ...
    if ($content_sub_auxiliary_id <= 0) {
        array_push($message, 'O campo "ContentSubAuxiliaryID", deve ser preenchido');
    }
    ...
    if (count($message) > 0) {
        ...
        $row = $contentSubAuxiliary->get($content_sub_auxiliary_id);

        /** Verifico se existe o registro **/
        if (isset($row)) {

            /** Verfico se o registro é válido **/
            if ($row->content_sub_auxiliary_id > 0) {

                /** Executo o método de exclusão **/
                if ($contentSubAuxiliary->delete($row->content_sub_auxiliary_id)) {
